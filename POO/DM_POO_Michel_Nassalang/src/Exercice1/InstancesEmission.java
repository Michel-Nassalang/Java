...
public class InstancesEmission {
	...
	public static void main(String[] args) {
		...
		for(int i=0; i<len; i++) { // superposition des programmes
			for(int j=0; j<len; j++) {
				if(i!=j && proTele[i].heurefin() > proTele[j].getHdiffuse() && proTele[i].getHdiffuse() < proTele[j].getHdiffuse()) {
					System.out.println("Les emissions \"" + proTele[i].getNom() + "\" de type " + proTele[i].type() + " et \"" + proTele[j].getNom() + "\" de type " + proTele[j].type() + " programmées sont superposées");
					System.out.println("\"" + proTele[i].getNom() + "\"   Début: " + proTele[i].getHdiffuse() + "h    Fin: " + proTele[i].heurefin() + "h");
					System.out.println("\"" + proTele[j].getNom() + "\"   Début: " + proTele[j].getHdiffuse() + "h    Fin: " + proTele[j].heurefin() + "h");
					sp = sp + 2;
				}
			}
		}
			...
			for(int j=0; j<len; j++) {
				if(proTele[j].getHdiffuse() == i) {
					for(int k=0; k<(proTele[j].getDuree()); k++) {
						int l = i+k;
						hours.add(l);
						System.out.println("A " + l + "h on a l'émission " + proTele[j].type() +" \"" + proTele[j].getNom() + "\"; ");
					}
				}
			}
			if(!hours.contains(i)) {
