...
class Pharmacie {
 ...
 public static void affichage(Client[] clients, Medicament[] medicaments) {
	...
	for(int i=0; i<clients.length;i++) {
		System.out.println("- le client " + clients[i].getNom() + " avec " + clients[i].getCredit() + " au niveau de la pharmacie");
	}
	System.out.println("                 -----------------");
	System.out.println("Les médicaments disponibles dans la pharmacie sont :");
	for(int i=0; i<medicaments.length;i++) {
		System.out.println("- le medicament " + medicaments[i].getNom() + " avec " + medicaments[i].getStock() + " d'exemplaires disponibles qui coûte " + medicaments[i].getPrix() + " euros l'euro.");
	}
}
